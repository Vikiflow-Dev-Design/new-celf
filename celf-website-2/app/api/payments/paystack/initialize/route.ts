  ...
  try {
    ...
    if (!process.env.PAYSTACK_SECRET_KEY) {
      return NextResponse.json(
        { error: "PAYSTACK_SECRET_KEY not set in environment" },
        { status: 500 }
      );
    }
    ...
    const supported = ["NGN", "USD"];
    const cur = String(currency || "NGN").toUpperCase();
    if (!supported.includes(cur)) {
      return NextResponse.json(
        { error: `Unsupported currency. Use one of: ${supported.join(", ")}` },
        { status: 400 }
      );
    }
    ...
    if (!res.ok) {
      return NextResponse.json(
        { error: data?.message || "Failed to initialize Paystack" },
        { status: res.status }
      );
    }
